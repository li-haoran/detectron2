...
void offsetAccumulate_im2col(const at::Tensor data_dcn, const at::Tensor data_target,
                             const int num_target, const int num_offset, 
                             const int height_out, const int width_out, const int height_in,
...
void offsetAccumulate_col2im(const at::Tensor grad_output,  
                              ...
                              const int num_offset,const int height_out, const int width_out,
                              const int height_in,
                              const int width_in, float ratio_h,float ratio_w,
...
void offsetAccumulate_col2im_coord(const at::Tensor grad_output,  const at::Tensor data_dcn, 
                              const at::Tensor data_target, const int num_target, 
                              const int num_offset,const int height_out, 
                              const int width_out, const int height_in,
                              const int width_in, float ratio_h,float ratio_w,
...
void points_collect_forward_cuda(at::Tensor target_offset, at::Tensor dcn_offset, at::Tensor output)
{
 ...
 for (int b = 0; b < batch; b++)
  {
    offsetAccumulate_im2col(dcn_offset[b], target_offset[b], 
                            num_target,num_offset,
                            height_out,width_out,
                            height_in,width_in,ratio_h,ratio_w, output[b]);
                            // channels_out means the target points size.
}
}

void points_collect_backward_cuda(
    at::Tensor target_offset, at::Tensor dcn_offset,
    ...
    at::Tensor grad_output)
{
  ...
  grad_output =
      grad_output.view({grad_output.size(0),grad_output.size(1), grad_output.size(2), grad_output.size(3)});
  ...
  for (int b = 0; b < batch; b++)
  {
    ...
    offsetAccumulate_col2im(grad_output[b],target_offset[b], 
                            num_target,num_offset,
                            height_out,width_out,
                            height_in,width_in,ratio_h,ratio_w,grad_dcn_offset[b]);

    offsetAccumulate_col2im_coord(grad_output[b],dcn_offset[b],target_offset[b], 
                            num_target,num_offset,
                            height_out,width_out,
                            height_in,width_in,ratio_h,ratio_w,grad_target_offset[b]);
  }
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m)
{
  m.def("points_collect_forward_cuda", &points_collect_forward_cuda,
        "points collect forward (CUDA)");
  m.def("points_collect_backward_cuda", &points_collect_backward_cuda,
