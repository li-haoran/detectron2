...
class _NonLocalNd_bn(nn.Module):
    def __init__(self, dim, inplanes, ratio, downsample, use_gn, lr_mult, use_out, out_bn, whiten_type, temperature, with_gc, with_2fc, double_conv,with_state,Ncls):
        ...
        if with_state:
            self.state_conv=conv_nd(inplanes,Ncls,1,bias=False)
    ...
    def forward(self, x):
        ...
        if not('norm' in self.whiten_type):
            sim_map = sim_map/self.scale

        if self.with_state:
            p=self.state_conv(x)
            p=p.view(p.size(0),p.size(1),-1,1)
            sim_map=sim_map.unsqueeze(1)

            sim_map=sim_map-p
            sim_map=sim_map**2
            sim_map=torch.exp(-sim_map)
            sim_map,_=sim_map.max(dim=1)
...
class NonLocal2dGc(_NonLocalNd_bn):
    def __init__(self, inplanes, ratio, downsample=True, use_gn=False, lr_mult=None, use_out=False, out_bn=False, whiten_type=['channel'], temp=1.0, with_gc=False, with_2fc=False, double_conv=False,with_state=False,Ncls=64):
        super(NonLocal2dGc, self).__init__(dim=2, inplanes=inplanes, ratio=ratio, downsample=downsample, use_gn=use_gn, lr_mult=lr_mult, use_out=use_out, out_bn=out_bn, whiten_type=whiten_type, temperature=temp, with_gc=with_gc, with_2fc=with_2fc, double_conv=double_conv,with_state=with_state,Ncls=Ncls)
